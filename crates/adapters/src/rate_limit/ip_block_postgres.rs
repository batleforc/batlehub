...
impl IpBlockStore for PgIpBlockStore {
    async fn record_violation(&self, ip: &str, window_secs: u32) -> Result<(u64, u64), CoreError> {
        ...
        let now = now_unix();
        let (window_start_u64, window_reset) = super::violation_window(now, window_secs);
        let window_start = window_start_u64 as i64;
...
// Window alignment is tested in rate_limit::mod::tests via the shared violation_window helper.
