...
2. **Whether the catalog stays a single route with a facet, or splits cached/upstream results.**
   `PackageExplorer` currently interleaves cached entries and upstream search results with a `kind`
   discriminator. Merging into one catalog inherits that ambiguity and should resolve it during
   Phase 3.
3. **Whether `AdminDashboard` needs API data it does not have** (per-registry savings over time,
   cache hit-rate trend). If yes, that is a separate API RFC, not a quiet addition here.
4. **French copy authorship.** §4.6 rules out machine translation for domain terms. Who writes and
...
| 2 | **Token layer.** `ui/src/design/tokens.css` derived from `DESIGN.md` and the proof, the `@theme` remap, the two R10 corrections, contrast unit tests over every token pair in both themes, `task ui:tokens` and the `website/` copy + drift check. | Yes — fixes §2.7 and the contrast half of §2.5 independently. |
| 3 | **Primitives and states.** Re-cut the 18 primitives; add `Skeleton`, `EmptyState`, `Announcer`, `Breadcrumb`, `Facet`, `DestructiveConfirm`. Land the §4.7 keyboard, focus, landmark and reduced-motion work in the component layer. | Yes — every existing page inherits the a11y fixes. |
| 4 | **Shell and IA.** Identity-driven nav, the `/`-home split, `LEGACY_REDIRECTS`, `/me` and `/tools` hubs, all §9 redirects and their router tests. | Yes — the navigation fix stands alone. |
| 5 | **Catalog and package pages.** Merge `PackageList` + `PackageExplorer`; merge the three detail pages into `/packages/:registry/:name` with inline admin controls. Resolve open question 3. | Yes — the single biggest usability win in §2. |
| 6 | **Admin surfaces.** Re-cut the 17 admin pages against the new system; split `AdminPackages` and `AdminConfigReload`; rebuild `AdminDashboard` around the operator's real question. | Partially — best after 3 and 4. |
| 7 | **Setup and first run.** The connect-a-tool flow, the fresh-instance path, the 403 → `/tools` link, and every empty state's copy. | Yes. |
| 8 | **i18n extraction and the French catalogue.** Strings extracted per phase as pages are rebuilt; this phase closes the gap, adds the key-parity test and the literal-string lint gate. | No — it is the closing gate on §4.6. |
| 9 | **Verification and CI.** Rendered-page detector pass at both viewports, axe pass, `website/` anti-pattern fixes, docs screenshot refresh. | No — it is the acceptance phase. |
