...
coordinate: a Maven version is a set of files, and a Terraform provider is
addressed by OS and architecture as well as version, so "fetch this version" has
no single meaning for either. The page says which rather than showing a disabled
button — see [fetching from the
console](/guide/admin-config#console-fetch).
...
| pypi | the metadata document, else the artifact | yes | versions + README | yes |
| conda | a file inside the artifact | yes | versions only | yes |
