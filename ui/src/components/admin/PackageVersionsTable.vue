...
import {
  blockPackage, unblockPackage, bulkBlockPackages, bulkUnblockPackages, invalidatePackage,
} from "@/client/sdk.gen";
...
function isPreRelease(v: string) { return v.includes("-"); }
...
function viewArtifact(v: PackageVersionDetail) {
  router.push({
    ...
    query: {
      registry: props.registry, name: props.name, version: v.version,
...
async function doBlock(v: PackageVersionDetail) {
  ...
  if (!reason) return;
  await blockPackage({ body: { registry: props.registry, name: props.name, version: v.version, artifact: v.artifact ?? undefined, reason } });
  emit("reload");
}

async function doUnblock(v: PackageVersionDetail) {
  await unblockPackage({ body: { registry: props.registry, name: props.name, version: v.version, artifact: v.artifact ?? undefined } });
  emit("reload");
}
...
async function doInvalidate(v: PackageVersionDetail) {
  ...
  await invalidatePackage({ body: { registry: props.registry, name: props.name, version: v.version, artifact: v.artifact ?? undefined } });
  emit("reload");
}
...
async function bulkBlock() {
  ...
  try {
    const res = await bulkBlockPackages({
      body: { items: selected.value.map(v => ({ registry: props.registry, name: props.name, version: v.version, artifact: v.artifact ?? null, reason })) },
    });
    const r = res.data;
    if (r) bulkMsg.value = `Blocked ${r.succeeded_count} version(s)${r.failed_count ? `, ${r.failed_count} failed` : ""}.`;
  } finally {
...
async function bulkUnblock() {
  ...
  try {
    const res = await bulkUnblockPackages({
      body: { items: selected.value.map(v => ({ registry: props.registry, name: props.name, version: v.version, artifact: v.artifact ?? null })) },
    });
    const r = res.data;
    if (r) bulkMsg.value = `Unblocked ${r.succeeded_count} version(s)${r.failed_count ? `, ${r.failed_count} failed` : ""}.`;
  } finally {
    bulkLoading.value = false;
    selectedIds.value = new Set();
    emit("reload");
...
<template>
  ...
  >
    ...
    <Button size="sm" variant="destructive" :disabled="bulkLoading" @click="bulkBlock">Block selected</Button>
    <Button size="sm" variant="outline" :disabled="bulkLoading" @click="bulkUnblock">Unblock selected</Button>
    ...
    <CardContent class="p-0">
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead class="w-8">
              <input type="checkbox" :checked="allSelected" class="cursor-pointer" @change="toggleAll">
            </TableHead>
        ...
        <TableBody>
          ...
          >
            <TableCell class="w-8">
              <input type="checkbox" :checked="selectedIds.has(v.id)" class="cursor-pointer" @change="toggle(v)">
            </TableCell>
            ...
            <TableCell class="text-right">
              <div class="flex justify-end gap-2">
                ...
                <Button v-if="v.cached" variant="outline" size="sm" @click="doInvalidate(v)">Purge cache</Button>
                <Button v-if="v.status.status === 'blocked'" variant="outline" size="sm" @click="doUnblock(v)">Unblock</Button>
