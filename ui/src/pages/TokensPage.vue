...
const { data: tokens, loading, error, reload } = useApi<any[]>(() => listTokens(), []);
...
async function submitCreate() {
  ...
  try {
    ...
    if (apiError) {
      createError.value = (apiError as any)?.message ?? "Failed to create token.";
    } else {
      showCreate.value = false;
      newToken.value = (data as any)?.token ?? null;
      newTokenExpiry.value = (data as any)?.expires_at ?? null;
      reload();
      startAutoClear();
    }
  } finally {
...
async function revokeToken(id: string) {
  ...
  try {
    ...
    if (apiError) {
      revokeError.value = (apiError as any)?.message ?? "Failed to revoke token.";
    } else {
      reload();
    }
  } finally {
    ...
    <Alert v-if="newToken" variant="success" class="relative">
      ...
      <div class="pl-2 space-y-2">
        ...
        <div class="flex items-center gap-2">
          ...
          <Button variant="outline" size="icon" @click="copyToken" class="shrink-0 h-8 w-8">
        ...
        <Button variant="ghost" size="sm" @click="dismissToken" class="h-7 text-xs">
        ...
        <Table v-else>
          ...
          <TableBody>
            <TableRow v-for="tok in tokens" :key="tok.id">
              <TableCell class="font-medium">{{ tok.name }}</TableCell>
              <TableCell>
    ...
    <Dialog :open="showCreate" @update:open="showCreate = $event">
      <template #default>
        <div class="space-y-1 pr-6">
          ...
          <p class="text-sm text-muted-foreground">Choose a name, role, and lifetime for your token.</p>
        </div>
        ...
        <div class="space-y-4 mt-2">
          ...
          <div class="flex justify-end gap-2 pt-2">
            <Button variant="outline" @click="showCreate = false" :disabled="creating">Cancel</Button>
