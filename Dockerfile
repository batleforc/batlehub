...
RUN cargo build --release -p proxy-cache-server 2>/dev/null; exit 0
...
RUN cargo build --release -p proxy-cache-server
...
COPY --from=builder /build/target/release/proxy-cache /usr/local/bin/proxy-cache
COPY config.example.toml /etc/proxy-cache/config.toml
RUN proxy-cache --config /etc/proxy-cache/config.toml dump-spec > openapi.json && \
    npm run generate && \
    npm run build
...
ENTRYPOINT ["proxy-cache"]
CMD ["--config", "/etc/proxy-cache/config.toml"]
